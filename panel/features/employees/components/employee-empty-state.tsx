...
export function EmployeeEmptyState({ mode, onResetFilters }: EmployeeEmptyStateProps) {
  const router = useRouter()

  if (mode === 'no-results') {
    return (
      <div className="flex flex-col items-center justify-center p-8 py-12 text-center rounded-2xl border border-dashed border-neutral-800 bg-card/25 backdrop-blur-md animate-in fade-in duration-300">
  ...
  return (
    <div className="flex flex-col items-center justify-center p-8 py-16 text-center rounded-2xl border border-dashed border-neutral-800 bg-card/25 backdrop-blur-md animate-in fade-in duration-300">
      ...
      <p className="text-xs text-muted-foreground max-w-sm leading-normal mb-6">
        İlk çalışanınızı oluşturarak ekip yönetimini başlatın.
      </p>
      <Button
