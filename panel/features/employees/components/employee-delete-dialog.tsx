          ...
          {isAssignedToBrands ? (
            <div className="space-y-3">
              <p className="text-xs text-muted-foreground leading-normal">
                <strong className="text-foreground">"{employeeName}"</strong> isimli çalışan silinemiyor çünkü şu anda aktif marka ekiplerinde görev alıyor.
              </p>
              <div className="p-3 bg-amber-500/10 text-amber-400 rounded-xl border border-amber-500/20 text-xs font-semibold leading-normal">
                Bu çalışan bir veya daha fazla markaya atanmış. Silmeden önce marka atamalarını kaldırın.
                <span className="block mt-1.5 font-normal text-muted-foreground text-[10px]">
                  Atanmış olduğu marka sayısı: {assignedBrandsCount}
                </span>
              </div>
            </div>
          ) : (
        ...
        <div className="px-6 py-4 bg-muted/20 border-t border-neutral-800 flex justify-end gap-3">
          {isAssignedToBrands ? (
            <Button
              type="button"
              onClick={onClose}
              className="h-9 text-xs px-4 bg-neutral-800 hover:bg-neutral-700 text-foreground font-semibold border"
            >
              Tamam
            </Button>
          ) : (
