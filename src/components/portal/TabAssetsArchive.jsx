...
export default function TabAssetsArchive({ customer }) {
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="relative overflow-hidden p-5 sm:p-6 rounded-3xl bg-gradient-to-r from-cyan-950/40 via-slate-900 to-slate-950 border border-cyan-500/30 backdrop-blur-2xl shadow-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-[11px] font-black uppercase tracking-wider text-cyan-400">Bulut Entegrasyonu</span>
            ...
            <h3 className="text-base font-black text-white mt-0.5">4K Master Bulut Arşivi & Dosya Teslim Havuzu</h3>
            <p className="text-xs text-slate-400 mt-1">
              Google Drive ve WeTransfer entegrasyonlu yüksek hızlı dosya arşivi ve bulut teslim motoru çok yakında tüm müşterilerimize açılacaktır.
            </p>
          </div>
        </div>
        <div className="shrink-0 flex items-center gap-2 self-start sm:self-center">
          <span className="text-xs font-bold text-cyan-300 bg-cyan-500/10 px-3.5 py-2 rounded-xl border border-cyan-500/30">
            ⏳ Çok Yakında Aktif
          </span>
        </div>
      </div>

      {/* 1. DELIVERED 4K MASTER EXPORTS (GOOGLE DRIVE / CLOUD) [3, E] - YAKINDA */}
      <div className="bg-slate-900/80 border border-slate-800 p-6 rounded-3xl backdrop-blur-xl shadow-xl space-y-5 opacity-90">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <span className="text-xs font-extrabold text-amber-300 bg-amber-500/10 border border-amber-500/30 px-3 py-1 rounded-xl self-start sm:self-center">
            ⏳ Yakında Aktif
          </span>
        </div>
          ...
          {deliveredDrivePackages.map((pkg) => (
            ...
            >
              ...
              <div className="flex items-center justify-between pt-3 border-t border-slate-900">
                ...
                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-900 text-slate-400 rounded-xl text-xs font-bold border border-slate-800">
                  <Clock className="w-3.5 h-3.5 text-amber-400" />
                  <span>Yakında İndirilebilir</span>
                </span>
              </div>
            </div>
          ))}
      ...
      {/* 2. PRESENTATIONS & PDF REPORTS ARCHIVE [3, E] - YAKINDA */}
      <div className="bg-slate-900/80 border border-slate-800 p-6 rounded-3xl backdrop-blur-xl shadow-xl space-y-5 opacity-90">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Ajansınız tarafından hazırlanan resmi PDF strateji ve performans dokümanları arşivi hazırlanıyor
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {presentationsAndReports.map((doc) => (
            ...
            >
              ...
              <span className="px-3 py-1.5 bg-slate-900 text-slate-400 rounded-xl text-xs font-bold border border-slate-800 flex items-center gap-1 shrink-0">
                <Clock className="w-3.5 h-3.5 text-amber-400" />
                <span>Yakında</span>
              </span>
            </div>
      ...
      {/* 3. CLIENT DRIVE & ASSET TRANSMISSION POOL [3, E] - YAKINDA */}
      <div className="bg-gradient-to-r from-slate-900 via-purple-950/20 to-slate-900 border border-purple-500/30 p-6 rounded-3xl backdrop-blur-xl shadow-xl space-y-5 opacity-90">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            <h3 className="text-base font-black text-white flex items-center gap-2">
